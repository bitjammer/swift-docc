...
struct ArticleSymbolMentions {
    ...
    ///
    /// - Note: Keep this private, as the sorting implementation should remain flexible
    ///   and transparent to the renderer.
    private var mentions: [ResolvedTopicReference: [ResolvedTopicReference: Int]] = [:]

    /// Record a symbol mention within an article.
    mutating func article(_ article: ResolvedTopicReference, didMention symbol: ResolvedTopicReference) {
        mentions[symbol, default: [:]][article, default: 0] += 1
    }
    ...
    func visitSymbolLink(_ symbolLink: SymbolLink) {
        if let destination = symbolLink.destination,
           let symbol = context.referenceIndex[destination] {
            print("Article \(article.url) ==> \(destination)")
            context.articleSymbolMentions.article(article, didMention: symbol)
        }
    }
}
